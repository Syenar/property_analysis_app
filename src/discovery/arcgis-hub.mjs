...
function collectServiceUrls(value, out = new Set(), depth = 0) {
  ...
  if (typeof value === 'object') for (const entry of Object.values(value)) collectServiceUrls(entry, out, depth + 1);
  return out;
}

function officialByHost(url) {
  try { const host = new URL(url).hostname.toLowerCase(); return host.endsWith('.gov') || host.endsWith('.us'); }
  catch { return false; }
}

export class ArcGISHubResolver {
  constructor({ http, maxItems = 12 } = {}) { this.http = http; this.maxItems = maxItems; }
  ...
  async resolveResults(results, jurisdiction) {
    ...
    for (const entry of ids) {
      try { candidates.push(...await this.resolveItem(entry.id, jurisdiction, entry.result)); }
      catch {}
    }
    return candidates;
  }
  ...
  async resolveItem(itemId, jurisdiction, result = {}) {
    ...
    if (!itemUrl || /web map|web mapping application|feature collection/i.test(item.type || '')) {
      try { const data = await this.http.getJson(`${base}/data?f=json`); collectServiceUrls(data, urls); }
      catch {}
    }
    return [...urls].map((url) => {
      const candidate = {
        platform: 'arcgis', discoverySource: 'arcgis-hub-item', itemId,
        title: item.title || result.title || url,
        description: item.description || item.snippet || result.description || '',
        owner: item.owner || null, access: item.access || 'public',
        modified: item.modified ? new Date(item.modified).toISOString() : null,
        itemType: item.type || null, tags: item.tags || [], extent: item.extent || null,
        url, official: officialByHost(url)
      };
      const authority = sourceAuthorityScore(candidate, jurisdiction);
      return { ...candidate, freshness: sourceFreshness(candidate.modified), lifecycle: authority.lifecycle, confidence: authority.score, confidenceReasons: authority.reasons };
    });
  }
}
