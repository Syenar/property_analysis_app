...
function featureDistanceMeters(feature, coordinates) {
  ...
  let best = Infinity;
  for (const polygon of polygons) for (const ring of polygon) for (let i = 1; i < ring.length; i++) best = Math.min(best, pointSegmentDistanceMeters(point, ring[i - 1], ring[i]));
  return best;
}

function chooseNearbyParcel(features, inputAddress, coordinates = null) {
  ...
  return { feature:null, reason:'ambiguous', count:features.length, bestAddressScore:ranked[0]?.score || 0, nearestMeters:Number.isFinite(nearest[0]?.distance) ? nearest[0].distance : null };
}
...
export class PropertyResearchEngine {
  ...
  async run(address, { fetchOrdinanceDocuments = true, maxGisCandidates = 8, maxOrdinanceDocuments = 3, onProgress = null } = {}) {
    ...
    await emit('sources', 'active', 'Checking known sources and discovering public GI